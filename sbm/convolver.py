...
class Convolver:
    ...
    def __init__(self, alm: np.ndarray, nside: int, spin_k: float,use_hwp):
        """Initialize the class with alm or blm and nsidespin data
    ...
    def _make_blk(self, k):
        ...
        blm_spin = self._eb2spin()
        if self.use_hwp is False:
            blk = np.zeros((3,self.lmax+1),np.complex64)
            blk[0,k:] = blm_spin[0,idx_start:idx_stop+1]*np.sqrt(4*np.pi/(2*ell[k:]+1))
            blk[1,k:] = blm_spin[1,idx_start:idx_stop+1]*np.sqrt(4*np.pi/(2*ell[k:]+1))
            blk[2,k:] = blm_spin[2,idx_start:idx_stop+1]*np.sqrt(4*np.pi/(2*ell[k:]+1))
            return blk
        else:
            return NotImplemented
        
    def _make_clm(self,blk,k):
        ...
        Returns:
            transfer function of the beam corresponding to spin k
        """
        alm_spin = self._eb2spin()
        if self.use_hwp is False:
            s0_pk_clm = hp.almxfl(alm_spin[0,:],blk[0,:])
            s2_pk_clm = hp.almxfl(alm_spin[1,:],blk[2,:]) + hp.almxfl(alm_spin[2,:],blk[1,:])
            s0_mk_clm = (hp.almxfl(alm_spin[0,:],blk[0,:]))*(-1)**k 
            s2_mk_clm = (hp.almxfl(alm_spin[1,:],np.conj(blk[1,:])) + hp.almxfl(alm_spin[2,:],np.conj(blk[2,:])))*(-1)**k 
            return [-(s0_pk_clm+s0_mk_clm)/2, -(s0_pk_clm-s0_mk_clm)/(2j)],[-(s2_pk_clm+s2_mk_clm)/2, -(s2_pk_clm-s2_mk_clm)/(2j)]
        else:
            return NotImplemented
    ...
    def __mul__(self, other):
        ...
        if self.use_hwp is False:
            for k_idx, k in enumerate(self.spin_k):
                blk = other._make_blk(k)
                clm = self._make_clm(blk, k)
                if k == 0:
                    s0_sk_map = hp.alm2map(-clm[0][0], nside = self.nside,lmax = self.lmax, mmax = self.lmax)
                    s2_sk_map = hp.alm2map(-clm[1][0], nside = self.nside,lmax = self.lmax, mmax = self.lmax)
                    all_maps[k_idx,0,:] = s0_sk_map
                    all_maps[k_idx,1,:] = s2_sk_map
                elif k > 0:
                    s0_sk_map = hp.alm2map_spin([clm[0][0],clm[0][1]], nside = self.nside, spin = k, lmax = self.lmax, mmax = self.lmax)
                    s2_sk_map = hp.alm2map_spin([clm[1][0],clm[1][1]], nside = self.nside, spin = k, lmax = self.lmax, mmax = self.lmax)
                    all_maps[k_idx,0,:] = s0_sk_map[0] + s0_sk_map[1]*1j
                    all_maps[k_idx,1,:] = s2_sk_map[0] + s2_sk_map[1]*1j
                elif k < 0:
                    s0_sk_map = hp.alm2map_spin([clm[0][0],clm[0][1]], nside = self.nside, spin = abs(k), lmax = self.lmax, mmax = self.lmax)
                    s2_sk_map = hp.alm2map_spin([clm[1][0],clm[1][1]], nside = self.nside, spin = abs(k), lmax = self.lmax, mmax = self.lmax)
                    all_maps[k_idx,0,:] = s0_sk_map[0] - s0_sk_map[1]*1j
                    all_maps[k_idx,1,:] = s2_sk_map[0] - s2_sk_map[1]*1j
            return all_maps
...
def elliptical_beam(nside: int, fwhm: float, q: float):
    ...
    result = 1/(2*np.pi*sigma**2*q**2)*np.exp(-(np.cos(phi)**2 + q**2 *np.sin(phi)**2)*(theta**2/(2*(sigma**2)*(q**2))))
    maps[0] = result
