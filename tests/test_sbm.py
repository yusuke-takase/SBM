...
class TestSBM(unittest.TestCase):
    ...
    def test_diff_gain(self, save_output_map=save):
        ...
        for mdim in mdims:
            ...
            if save_output_map==True:
                ...
                reference = hp.read_map(f"tests/reference/diff_gain_output_map_mdim_{mdim}.fits", field=(0,1,2))
    ...
    def test_diff_pointing(self, save_output_map=save):
        ...
        for mdim in mdims:
            ...
            if save_output_map==True:
                ...
                reference = hp.read_map(f"tests/reference/diff_pointing_output_map_mdim_{mdim}.fits", field=(0,1,2))
                self.assertTrue(np.allclose(output_map, reference))
    ...
    def test_abs_pointing(self, save_output_map=save):
        ...
        for mdim in mdims:
            ...
            if save_output_map==True:
                hp.write_map(f"tests/reference/abs_pointing_output_map_mdim_{mdim}.fits", output_map, overwrite=True)
                print("Absolute pointing offset with HWP output map is saved.")
            else:
                reference = hp.read_map(f"tests/reference/abs_pointing_output_map_mdim_{mdim}.fits", field=(0,1,2))
                self.assertTrue(np.allclose(output_map, reference))
    ...
    def test_hwpip(self, save_output_map=save):
        ...
        mdims = [3]
        for mdim in mdims:
            signal_field = SignalFields.hwp_ip_field(self.scan_field, mdim, epsilon, phi_qi, self.I)
            output_map = self.scan_field.map_make(signal_field)
            if save_output_map==True:
                hp.write_map(f"tests/reference/hwpip_output_map_mdim_{mdim}.fits", output_map, overwrite=True)
                print("HWP-IP output map is saved.")
            else:
                reference = hp.read_map(f"tests/reference/hwpip_output_map_mdim_{mdim}.fits", field=(0,1,2))
                self.assertTrue(np.allclose(output_map, reference))
    ...
    def test_noise_generation_with_HWP(self, save_output_map=save):
        ...
        for spin_n_basis, spin_m_basis in zip(spin_n_basis_list, spin_m_basis_list):
            ...
            if save_output_map==True:
                hp.write_map(f"tests/reference/noise_map_{net_ukrts}ukrts_hwp_mdim_{mdim}.fits", output_map, overwrite=True)
                print("Noise map is saved.")
            else:
                reference = hp.read_map(f"tests/reference/noise_map_{net_ukrts}ukrts_hwp_mdim_{mdim}.fits", field=(0,1,2))
                self.assertTrue(np.allclose(output_map, reference))
    ...
    def test_noise_generation_without_HWP(self, save_output_map=save):
        ...
        for spin_n_basis, spin_m_basis in zip(spin_n_basis_list, spin_m_basis_list):
            ...
            output_map = self.scan_field.generate_noise(spin_n_basis, spin_m_basis, seed)
            if save_output_map==True:
                hp.write_map(f"tests/reference/noise_map_{net_ukrts}ukrts_mdim_{mdim}.fits", output_map, overwrite=True)
                print("Noise map is saved.")
            else:
                reference = hp.read_map(f"tests/reference/noise_map_{net_ukrts}ukrts_mdim_{mdim}.fits", field=(0,1,2))
                self.assertTrue(np.allclose(output_map, reference))
